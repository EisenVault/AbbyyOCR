...
(function() {
	 ...
	 * YUI Library aliases
	 */
	var Dom = YAHOO.util.Dom, Event = YAHOO.util.Event, Anim = YAHOO.util.Anim;
	 ...
	 * 
	 * @param name
	 *            {String} The name of the TableViewRenderer
	 * @return {Alfresco.DocumentListTableViewRenderer} The new
	 *         TableViewRenderer instance
	 * @constructor
	 */
	Alfresco.DocumentListTableViewRenderer = function(name, parentDocumentList,
			jsonConfig) {
		Alfresco.DocumentListTableViewRenderer.superclass.constructor.call(
				this, name, parentDocumentList);
	 ...
	 */
	Alfresco.DocumentListTableViewRenderer.prototype.renderView = function DL_SVR_renderView(
			scope, sRequest, oResponse, oPayload) {
		...
		Dom.addClass(container, 'alf-table');
		Dom.removeClass(Dom.get(scope.id + "-table-config-button-container"),
				'hidden');

		// Always hide the fileName column...
		// This has been done due to time-constraints and the issues surrounding
		// the other views (most notably the detailed
		// and simple views) changing the widths of the fileName field. The
		// simplest solution was to just always hide this
		// field and rely on the JSON blob to add in columns that identify the
		// item.
		scope.widgets.dataTable.hideColumn("fileName");

		// Not all of the columns are properties, the selector, indicators and
		// actions are not properties and
		// need to be handled by specific configuration defined in the JSON
		// BLOB...
		if (this.additionalJsonConfig != null) {
			...
			// If a JSON blob has been provided then is should define the
			// columns that need to be
			// rendered in the detailed view. Iterate over this configuration
				...
				for (var i = 0; i < this.additionalJsonConfig.propertyColumns.length; i++) {
					...
					if (propColumn.property != null) {
						...
						var columnDef = {
							...
							formatter : (propColumn.link == "true") ? this
			...
			// In order to ensure that the actions column always appears at the
			// end we're goign to remove it and
			// then re-insert it. This is necessary because attempts at
			// inserting the additional columns before
			// the actions column always resulted in unexplainable errors and
			// this was the quickest solution to the
			// problem. It is however only necessary when the actions column is
			// going to be displayed and in
			// actual fact causes more errors if this is attempted after the
			// actions column is hidden...
			if (this.additionalJsonConfig.actions == null
					|| this.additionalJsonConfig.actions.show == "true") {
				var actionsColumn = scope.widgets.dataTable
			...
			YAHOO.util.Dom.setStyle(scope.id + this.parentElementIdSuffix,
					'display', '');
	 ...
	 */
	Alfresco.DocumentListTableViewRenderer.prototype.renderCellThumbnail = function DL_SVR_renderCellThumbnail(
		...
		if (YAHOO.env.ua.ie > 0) {
			try {
				...
				// Deliberately swallowing any generated exceptions without
				// remorse.
			}
			...
			if (node.properties["empfm:alltypes"] >=0 && node.properties["empfm:alltypes"] < 4) {
	                   elCell.innerHTML = '<span class="folder-small">'
									...
									+ (scope.dragAndDropEnabled ? '<span class="droppable"></span>'
											: '')
									+ Alfresco.DocumentList.generateFileFolderLinkMarkup(scope,
											record) + '<img id="' + imgId + '" src="'
	 ...
	 */
	Alfresco.DocumentListTableViewRenderer.prototype.destroyView = function DL_SVR_destroyView(
			scope, sRequest, oResponse, oPayload) {
		Alfresco.DocumentListTableViewRenderer.superclass.destroyView.call(
				this, scope, sRequest, oResponse, oPayload);

		// This gets called even before the view has first been created, so we
		// need to ensure we have the original
		// set of columns so that they can be restored for the benefit of other
		// views...
		if (!this.originalColumnDefinitions) {
			// Save the originalColumnDefinitions
			this.originalColumnDefinitions = [];
			var columnDefinitions = scope.widgets.dataTable.getColumnSet()
					.getDefinitions();
		...
		Dom.addClass(Dom.get(scope.id + "-table-config-button-container"),
				'hidden');

		// Rebuild the original columns...
		if (this.originalColumnDefinitions) {
			// Ensure that all the "core" columns are displayed again (these
			// columns are the ones that
			// were defined by the original detailed view and need to be in
			// place for the other views
			// to continue to work...
			scope.widgets.dataTable.showColumn("nodeRef");
			scope.widgets.dataTable.showColumn("status");
			scope.widgets.dataTable.showColumn("thumbnail");
			...
			var currentColumnDefinitionsCall = scope.widgets.dataTable
					.getColumnSet().getDefinitions();
			...
			for (var i = currentColumnDefinitionsCall.length - 1; i >= 0; i--) {
				...
				case "actions":
					break;
				default:
					scope.widgets.dataTable
							.removeColumn(currentColumnDefinitionsCall[i].key);
				}
			}

			// Add back in any of the original columns that have since been
			// deleted (typically this
			...
			for (var i = 0; i < this.originalColumnDefinitions.length; i++) {
				...
				default:
					scope.widgets.dataTable
							.insertColumn(this.originalColumnDefinitions[i]);
				}
			}
		}
	};
	 ...
	 *            {object|string}
	 */
	Alfresco.DocumentListTableViewRenderer.prototype.renderCellGenericString = function DL_SVR_renderCellGenericString(
			elCell, oRecord, oColumn, oData) {
		var record = oRecord.getData(), node = record.jsNode, properties = node.properties, propertyValue = properties[oColumn.field];
		if (propertyValue != null) {
			elCell.innerHTML = '<span class="alf-generic-property">'
					+ propertyValue + '</span>';
		}
	};

	/**
	 * Uses a defined metadata 'line' template and renderer as a custom datacell
	 * formatter
	 * 
	 ...
	 *            {object|string}
	 * @param line
	 *            {object} the metadata 'line' object
	 */
	Alfresco.DocumentListTableViewRenderer.prototype.renderCellMetadataLineRenderer = function DL_SVR_renderCellMetadataLineRenderer(
			scope, elCell, oRecord, oColumn, oData, line) {
		var desc = "", i, j, record = oRecord.getData(), jsNode = record.jsNode;

		var fnRenderTemplate = function fnRenderTemplate_substitute(p_key,
				p_value, p_meta) {
			...
			if (scope.renderers.hasOwnProperty(p_key)
				...
				if (jsNode.hasProperty(p_key)) {
					value = '<span class="item">' + label
							+ $html(jsNode.properties[p_key]) + '</span>';
				}
			}
		...
		if (!$isValueSet(line.view) || line.view == this.metadataLineViewName) {
			html = YAHOO.lang.substitute(line.template, scope.renderers,
					fnRenderTemplate);
			if ($isValueSet(html)) {
