...
/**
 * DocumentListSimpleViewRenderer component which extends
 * DocumentListViewRenderer
 * 
 * @namespace Alfresco
...
(function() {
	 ...
	 * 
	 * @param name
	 *            {String} The name of the SimpleViewRenderer
	 * @return {Alfresco.DocumentListSimpleViewRenderer} The new
	 *         SimpleViewRenderer instance
	 * @constructor
	 */
	Alfresco.DocumentListSimpleViewRenderer = function(name, parentDocumentList) {
		Alfresco.DocumentListSimpleViewRenderer.superclass.constructor.call(
	 ...
	 */
	YAHOO.extend(Alfresco.DocumentListSimpleViewRenderer,
	 ...
	 */
	Alfresco.DocumentListSimpleViewRenderer.prototype.renderCellThumbnail = function DL_SVR_renderCellThumbnail(
		 ...
		 containerTarget = new YAHOO.util.DDTarget(imgId); // Make the
			...
			elCell.innerHTML = '<span id="'
					...
					+ Alfresco.DocumentList.generateFileFolderLinkMarkup(scope,
							record) + '<img id="' + imgId + '" src="'
					+ Alfresco.constants.URL_RESCONTEXT
					+ 'components/images/filetypes/' + fileIcon + '" alt="'
					+ extn + '" title="' + $html(name) + '" /></a></span>';
	...
	/**
	 * Override Alfresco.DocumentListViewRenderer.setupRenderer to setup preview
	 * tooltip
	 */
	Alfresco.DocumentListSimpleViewRenderer.prototype.setupRenderer = function DL_SVR_setupRenderer(
			scope) {
		Dom
				.addClass(scope.id + this.buttonElementIdSuffix,
						this.buttonCssClass);
		// Tooltip for thumbnail in Simple View
		scope.widgets.previewTooltip = new YAHOO.widget.Tooltip(scope.id
				+ "-previewTooltip", {
			width : "108px"
		});
		scope.widgets.previewTooltip.contextTriggerEvent.subscribe(function(
				type, args) {
			var context = args[0], oRecord = scope.widgets.dataTable
					.getRecord(context.id), record = oRecord.getData();

			this.cfg.setProperty("text", '<img src="'
